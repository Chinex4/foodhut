    ...
    >
      <View className="flex-1 bg-black/30 justify-end">
        <View className="bg-white rounded-t-3xl p-4">
          <Text className="text-lg font-satoshiBold mb-3">Filters</Text>
          ...
          <View className="flex-row mb-3">
            {(["ALL", "MEALS", "KITCHENS"] as const).map((opt) => (
              <Pressable
                ...
                className={`mr-2 px-3 py-2 rounded-xl border ${scope === opt ? "bg-primary border-primary" : "bg-white border-neutral-200"}`}
              >
                <Text
                  className={`${scope === opt ? "text-white" : "text-neutral-700"} font-satoshiMedium`}
                >
                  {opt}
                </Text>
              </Pressable>
          ...
          {/* Page size */}
          <View className="mb-6">
            <Text className="text-neutral-600 mb-1">Results per page</Text>
            <TextInput
              value={perPage}
              onChangeText={setPerPage}
              keyboardType="number-pad"
              className="h-11 px-3 rounded-xl border border-neutral-200 font-satoshi"
              placeholder="20"
            />
          </View>
          ...
          <View className="flex-row justify-between">
            ...
            >
              <Text className="font-satoshiMedium text-neutral-800">
                Cancel
              </Text>
            </Pressable>
            <Pressable
